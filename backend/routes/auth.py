...
def register():
    ...
    if not is_strong_password(data['password']):
        return jsonify({'error': 'Password must be 8+ chars and include Uppercase, Lowercase, Number, and Special Character'}), 400
...
def change_password():
    ...
    if not is_strong_password(new_password):
        return jsonify({'error': 'New password must be 8+ chars and include Uppercase, Lowercase, Number, and Special Character'}), 400
