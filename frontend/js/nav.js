...
function renderSidebar(activeHref) {
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-footer">
            <div class="notif-bell-wrap" id="sidebarNotifBellWrap">
                <button class="notif-bell sidebar-bell" id="sidebarNotifBell"
                        onclick="openNotifModal()"
                        title="View Notifications"
                        aria-haspopup="dialog">
                    <i class="fas fa-bell"></i>
                    <span class="sidebar-notif-label">Notifications</span>
                    <span class="notif-badge" id="sidebarNotifBadge" style="display:none">0</span>
                </button>
            </div>
    ...
    injectToastContainer(); // Ensure toast container is ready
    initNotifications();
    loadPendingBadge();
}
...
function renderTopBell() {
    // Hide the top-bar container — the sidebar bell is the single notification entry point.
    // Injecting a second bell here caused duplicate IDs (notifBell, notifBadge) and double click handlers.
    const container = document.getElementById('top-notif-container');
    if (container) container.style.display = 'none';
}

/* ─────────────────────────────────────────────────────────
   TOAST SYSTEM — Automatic Corner Popups
───────────────────────────────────────────────────────── */
...
let _lastNotifId = null; // To track if we should show a toast

function initNotifications() {
    const user = getUser();
    if (!user) return;
    loadNotifications(user.id, true); // initial load, no toast
    if (_notifInterval) clearInterval(_notifInterval);
    _notifInterval = setInterval(() => loadNotifications(user.id), 15000); // 15s poll
}

async function loadNotifications(userId, isInitial = false) {
    try {
        const data = await apiGet(`/notifications/${userId}`);
        updateNotifBadge(data.unread_count);
        renderNotifModalBody(data.notifications);

        // Toast Logic: show toast if we have a NEW unread notification
        if (!isInitial && data.notifications.length > 0) {
            const latest = data.notifications[0];
            if (!latest.is_read && latest.id !== _lastNotifId) {
                _lastNotifId = latest.id;
                showToast("New Notification", latest.message, "info");
            }
        } else if (data.notifications.length > 0) {
            _lastNotifId = data.notifications[0].id;
        }
...
async function loadPendingBadge() {
    ...
    try {
        if (user.role === 'supervisor' || user.role === 'faculty') {
            // Use stored supervisor_id or fall back to /supervisor/all lookup
            let sv_id = user.supervisor_id;
            if (!sv_id) {
                try {
